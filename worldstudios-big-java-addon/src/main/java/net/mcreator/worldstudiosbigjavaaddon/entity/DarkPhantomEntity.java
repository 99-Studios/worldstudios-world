...
public class DarkPhantomEntity extends Monster {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new Goal() {
			...
			public void start() {
				...
				DarkPhantomEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
			}
			...
			public void tick() {
				...
				if (DarkPhantomEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					DarkPhantomEntity.this.doHurtTarget(this.getServerLevel(livingentity), livingentity);
				} else {
	...
	protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource source, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(serverLevel, new ItemStack(BigAddonModItems.DARK_SPLATTER.get()));
	}
	...
	public static void init(RegisterSpawnPlacementsEvent event) {
		event.register(BigAddonModEntities.DARK_PHANTOM.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
